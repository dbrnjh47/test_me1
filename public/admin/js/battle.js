$(document).ready(function() {
	...
	socket.on('battle.slider', function(data) {
		...
		setTimeout(function() {
			...
			<form action="https://api.random.org/verify" method='post' target='_blank'><input type='hidden' name='format' value='json'/><input type='hidden' name='random' value='${data.game.random}'/><input type='hidden' name='signature' value='${data.game.signature}' /><button style="width: 37px;
				height: 5px;
				margin-right: 11px;
				border: 0px;" class="item history_item history_${data.game.winner_team}" type="submit" data-hash=""></button>
			</form>`);
