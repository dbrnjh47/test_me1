$(document).ready(function() {
    ...
    socket.emit("jackpot init", room, (data) => {
        if(data.status) {
            ...
            setTimeout(function () {
                $('.roulette1').css('transform', 'translate3d(-'+ data.deg +'px, 0px, 0px)');
            }, 500);
    ...
    socket.on('jackpot', r => {
        ...
        if(r.type == 'timer' && room == r.room) {
            ...
            if(sec == 2 && r.data.nginxTime) {
                ...
                setTimeout(function(){ $('#animHide').removeClass('animated fadeOutUp');  $('#RuletkaSkrita').removeClass('animated bounceInRight');
                $('#gameCarousel').addClass('hidden'); }, 1000);
            }
			$('#timer').text(sec);
		}
        ...
        if(r.type == 'slider' && room == r.room) {
            ...
            setTimeout(function () {
                $('.roulette1').css('transform', 'translate3d(-'+ r.data.deg +'px, 0px, 0px)');
            }, 500);
