$(document).ready(function () {
	...
	socket.on('new.flip', function (data) {
		...
		var all = '';
			...
			all += '<p onclick="getUser('+ data.id +', 0)">'+ data.heads_from +' - '+ data.heads_to +' <svg class="icon"><use xlink:href="/img/symbols.svg#icon-ticket"></use></svg></p>';
			all += '</div>';
			all += '</div>';
			all += '</div>';
			all += '</div>';
			...
			all += '<span onclick="getUser('+ data.id +', 1)" class="val"><span>???</span> <svg class="icon"><use xlink:href="/img/symbols.svg#icon-ticket"></use></svg></span>';
			all += '</div>';
			all += '</div>';
			all += '<div class="hash">';
