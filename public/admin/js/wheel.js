$(document).ready(function() {
	...
	socket.on('wheel', function(data) {
        ...
        if(data.type == 'newGame') {
			...
			$('.game_Wheel .history_history').prepend(`
			<form action="https://api.random.org/verify" method='post' target='_blank'><input type='hidden' name='format' value='json'/><input type='hidden' name='random' value='${data.history.random}'/><input type='hidden' name='signature' value='${data.history.signature}' /><button style="width: 37px;
				height: 5px;
				margin-right: 11px;
				border: 0px;" class="item history_item history_${data.history.color}" type="submit" data-hash=""></button>
			</form>`);
